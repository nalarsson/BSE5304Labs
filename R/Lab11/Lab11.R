...
# fix Z to scale depending on basin size
...
for (i in 1:5) {
  objname=paste0("TIC0",i) #MUSLE$ID[i])
  print(objname)
  
  print(VSAParams$CN[i])  #[MUSLE$TIclass[i]])
  
  tmpdf=TMWB
  tmpdf = CNmodel(CNmodeldf = tmpdf, CNavg=VSAParams$CN[i], #MUSLE$TIclass[i]],
                  declat=myflowgage$declat,declon=myflowgage$declon)
  tmpdf$qpeak=tmpdf$Qpred/3600/24/1000*myflowgage$area/nTIclass*10^6    #MUSLE$areaSQKM*10^6 #m^3/sec
  tmpdf$sed=(tmpdf$Qpred*tmpdf$qpeak*myflowgage$area/nTIclass)^.56*MUSLE$KCPLSCFRG118[i]  #MUSLE$areaSQKM*100)^.56*MUSLE$KCPLSCFRG118[1] 
...
#this 5 times for TIC 1-4 where you will need the date, Qpred, and Average 
# Temperature
DPTI04=data.frame(date=TIC05$date,
                  Rt=TIC04$Qpred/1000, 
...
# We will take care of all of TIClass 05 now as will so 
# it makes sense when you repeat for TI Classes 1-4
# You will use muTS_TI01 thru muTS_TI02 to finish this lab
#
QS= 3.0 # A guess using the middle of the range 1-5
TR=20   # reference Temperature from Table 2.
...
# We will take care of all of TIClass 05 now as will so 
# it makes sense when you repeat for TI Classes 1-4
# You will use muTS_TI01 thru muTS_TI01 to finish this lab
#
QS= 3.0 # A guess using the middle of the range 1-5
TR=20   # reference Temperature from Table 2.
...
plot(TIC_loss); title("Average P Loss by TI Class (kg/day)")
dev.off()
