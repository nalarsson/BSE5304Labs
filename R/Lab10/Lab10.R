...
pacman::p_load(ggplot2,dplyr,patchwork,rnoaa,tidyr,magrittr)
...
#
setwd(srcdir)
#("package:EcoHydRology", unload = TRUE)
...
TICtotal=data.frame(TIC01$date, TIC01$sed,
                                TIC02$sed,
                                TIC03$sed,
                                TIC04$sed,
                                TIC05$sed)
TICtotal$TItotal=rowSums(TICtotal[,2:5])

setwd("~/2023/BSE5304Labs10/pdfs/Lab10")
attach(TICtotal)
pdf("ConstantPusle.pdf")
plot(Date,TItotal,type="l",xlab="",ylab="")
lines(Date,TI01,col="green")
lines(Date,TI02,col="purple")
lines(Date,TI03,col="blue")
lines(Date,TI04,col="navy")
lines(Date,TI05,col="darkgreen")
legend("topleft", title="TI Class",
       c("1","2","3","4","5","Total"), 
       fill=c("green","purple","blue","navy","darkgreen","black"))
title("Soil Loss Over Time - Constant Pusle", xlab="Time", ylab="Soil Loss")
dev.off()
detach(TICtotal)

rclconst=matrix(ncol=2)
rclconst=rbind(rclconst,c(1,mean(TIC01$sed)))
rclconst=rbind(rclconst,c(2,mean(TIC02$sed)))
rclconst=rbind(rclconst,c(3,mean(TIC03$sed)))
rclconst=rbind(rclconst,c(4,mean(TIC04$sed)))             
rclconst=rbind(rclconst,c(5,mean(TIC05$sed)))     
rclconst=rclconst[2:nrow(rclconst),]                

TIC_const=TIC
TIC_const=reclassify(TIC_const,rclconst)

pdf("ConstantPusleRaster.pdf")
plot(TIC_const)
title("Average Soil Loss - Constant Pusle")
dev.off()

#VARIABLE PUSLE VALUES
#change MUSLE$KCPLSCGRG118 to have varying values of P, so slightly different overall values. Right now, they are constant. 

MUSLE$slpPercent=MUSLE$slp*100

MUSLE$varPusle[1]=0.60
MUSLE$varPusle[2]=0.50
MUSLE$varPusle[3]=0.50
MUSLE$varPusle[4]=0.60
MUSLE$varPusle[5]=0.50

attach(MUSLE)
MUSLE$varKCPLSCFRG118=11.8*Kusle*Cusle*Pusle*LS*CFRG
detach(MUSLE)
...
legend("topleft", title="TI Class",
       c("1","2","3","4","5","Total"), 
       fill=c("green","purple","blue","navy","darkgreen","black"))
title("Soil Loss Over Time - Variable Pusle", xlab="Time", ylab="Soil Loss")
dev.off()
detach(TICtotal)



#plot values on raster
TIC_values=TIC
TIC_test=TIC

#reclassify function 
rclvar=matrix(ncol=2)
rclvar=rbind(rclvar,c(1,mean(TIC01$sed)))
rclvar=rbind(rclvar,c(2,mean(TIC02$sed)))
rclvar=rbind(rclvar,c(3,mean(TIC03$sed)))
rclvar=rbind(rclvar,c(4,mean(TIC04$sed)))             
rclvar=rbind(rclvar,c(5,mean(TIC05$sed)))     
rclvar=rclvar[2:nrow(rclvar),]                

pdf("VariablePusleRaster.pdf")
plot(TIC_const)
title("Average Soil Loss - Variable Pusle")
dev.off()
